...
```
openssl req -new -x509 -config "C:\openssl-0.9.8k_X64\openssl.cnf" -keyout ca-key -out ca-cert -days 365 -subj "/CN=local-security-CA"
```
...
```
kafka-console-producer.bat --broker-list localhost:9095,localhost:9096,localhost:9097 --topic test-topic --producer.config ../client-ssl.properties
```
...
```
kafka-console-producer.bat --broker-list localhost:9092,localhost:9093,localhost:9094 --topic test-topic
```
...
```
kafka-console-consumer.bat --bootstrap-server localhost:9092,localhost:9093,localhost:9094 --topic test-topic
```
